...
import error from '../assets/sounds/erro.mp3'
import soap from '../assets/sounds/soap_bubble.wav'
import spell from '../assets/sounds/bubble_spell.wav'

export default function Navbar(props){

    // const [play, {stop}] = useSound(error, {volume: 0.2})
    // const [play, {stop}] = useSound(soap, {volume: 0.1})
    ...
    return(
        <div className="fixed bg-gradient-to-r from-cyan-100 to-blue-500 opacity-25 hover:opacity-50 transition-all bottom-0 w-full h-24 flex items-center justify-evenly text-2xl">
                ...
                <div className="flex justify-around opacity-100 gap-10">
                    ...
                    <div className={`rounded-lg w-10 h-10 flex justify-center items-center ${page !== 'home' ? 'opacity-100' : 'opacity-0'} hover:bg-slate-900 hover:text-white transition-all group`}>
                    ...
                    <div className={`rounded-lg w-10 h-10 flex justify-center items-center ${page === 'education' ? 'bg-slate-900 text-white hover:bg-transparent hover:text-black' : 'hover:bg-slate-900 hover:text-white'}  transition-all group`} onMouseEnter={() => play({playbackRate: 1})} onMouseLeave={() => stop()}>
                    ...
                    <div className={`rounded-lg w-10 h-10 flex justify-center items-center ${page === 'experience' ? 'bg-slate-900 text-white hover:bg-transparent hover:text-black' : 'hover:bg-slate-900 hover:text-white'} transition-all group`} onMouseEnter={() => play({playbackRate: 1.1})} onMouseLeave={() => stop()}>
                    ...
                    <div className={`rounded-lg w-10 h-10 flex justify-center items-center ${page === 'personal' ? 'bg-slate-900 text-white hover:bg-transparent hover:text-black' : 'hover:bg-slate-900 hover:text-white'} transition-all group`} onMouseEnter={() => play({playbackRate: 1.2})}>
                ...
                <div className="flex justify-around opacity-100 gap-10">
                    <div className={`rounded-lg w-10 h-10 flex justify-center items-center hover:bg-slate-900 hover:text-white transition-all group`} onMouseEnter={() => play({playbackRate: 1.3})}>
                        <i class="fa-solid fa-chevron-left"></i>
                    </div>
                    <div className={`rounded-lg w-10 h-10 flex justify-center items-center hover:bg-slate-900 hover:text-white transition-all group`} onMouseEnter={() => play({playbackRate: 1.4})}>
                        {props.playing === true
                            ? (<i class="fa-solid fa-pause" onClick={() => props.setPlaying(false)}></i>)
                            : (<i class="fa-solid fa-play" onClick={() => props.setPlaying(true)}></i>)}
                        
                    </div>
                    <div className={`rounded-lg w-10 h-10 flex justify-center items-center hover:bg-slate-900 hover:text-white transition-all group`} onMouseEnter={() => play({playbackRate: 1.5})}>
                ...
                {/* socials */}
                <div className="flex justify-around opacity-100 gap-10">
                    <div className={`rounded-lg w-10 h-10 p-3 flex justify-center items-center hover:bg-slate-900 transition-all group`}>
                        <a href="https://www.are.na/darko-cejkov-t-ekov-cheykov" target={'_blank'}>
                            <svg className="hover:fill-white" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
                        ...
                        <span className="font-tabi absolute text-white  top-0 -m-10 scale-0 group-hover:scale-100 transition-all">Are.na</span>
                    </div>
...
const Icon = ({icon, route, text, value, setter}) => (
    <div className={`rounded-lg w-10 h-10 flex justify-center items-center ${value === route ? 'bg-slate-900 text-white hover:bg-transparent hover:text-black' : 'hover:bg-slate-900 hover:text-white'} transition-all group`}>
        <Link to={`/${route}`}  onClick={() => setter(route)}>
            {icon}
            {/* <i className="fa-regular fa-face-smile  h-100 w-100"></i> */}
        </Link>
        <span className="font-tabi absolute text-white  top-0 -m-10 scale-0 group-hover:scale-100 transition-all">{text}</span>
    </div>
)
